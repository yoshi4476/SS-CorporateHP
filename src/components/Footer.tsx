      ...
      <div className="mx-auto grid max-w-7xl gap-12 px-5 py-16 md:grid-cols-[1.3fr_1fr_1fr] md:py-20">
        <div>
          ...
          <p className="mt-4 text-sm text-white/60">
            TEL:{" "}
            <a href={`tel:${site.tel.replaceAll("-", "")}`} className="num text-white underline-offset-4 hover:underline">
              {site.tel}
            </a>
            <br />
            受付時間: {site.hours}
        ...
        <nav aria-label="事業内容">
          ...
          <ul className="grid gap-2.5 text-sm text-white/75">
            <li>
              <Link href="/services" className="font-bold transition-colors hover:text-white">
                事業内容一覧
              </Link>
            </li>
            {services.map((s) => (
        ...
        <div className="grid content-start gap-8">
          <nav aria-label="会社情報">
            ...
            <ul className="grid gap-2.5 text-sm text-white/75">
              ...
              <li>
                <a href={site.lpUrl} target="_blank" rel="noopener" className="transition-colors hover:text-white">
                  AI導入補助金 特設サイト ↗
                </a>
              </li>
              <li>
                <a href={site.labUrl} target="_blank" rel="noopener" className="transition-colors hover:text-white">
                  AI集客ラボ (運営メディア) ↗
                </a>
              </li>
              <li>
          ...
          <nav aria-label="最新のお知らせ">
            ...
            <ul className="grid gap-2.5 text-xs text-white/60">
              {news.slice(0, 2).map((n) => (
                <li key={n.slug}>
                  <Link href={`/news/${n.slug}`} className="transition-colors hover:text-white">
